...
def start_cron_job(env_vars):
    ...
    scheduler.add_job(force_run_job, 'cron', **parse_cron_expression(cron_expression))
    
    logger.info(f"Cron job set with expression: {cron_expression}")
    scheduler.start()
