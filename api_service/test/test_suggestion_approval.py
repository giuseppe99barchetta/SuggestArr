...
def test_owner_can_only_approve_own_suggestions():
    ...
    connection.executescript("""
        CREATE TABLE pending_requests (id INTEGER PRIMARY KEY, tmdb_id TEXT, media_type TEXT,
            status TEXT, owner_id INTEGER, job_id INTEGER, execution_id INTEGER,
        ...
        INSERT INTO pending_requests VALUES (1,'10','movie','awaiting_approval',7,3,NULL,NULL,NULL);
        INSERT INTO pending_requests VALUES (2,'20','movie','awaiting_approval',8,3,NULL,NULL,NULL);
    """)
...
def test_reject_and_blacklist_are_separate_actions():
    ...
    connection.executescript("""
        ...
        CREATE TABLE suggestion_blacklist (tmdb_id TEXT, media_type TEXT, created_by INTEGER,
            PRIMARY KEY (tmdb_id, media_type));
        INSERT INTO pending_requests VALUES (1,'10','movie','awaiting_approval',7,3,NULL,NULL,NULL);
        INSERT INTO pending_requests VALUES (2,'20','tv','awaiting_approval',7,3,NULL,NULL,NULL);
    """)
    queue = Queue(connection)
