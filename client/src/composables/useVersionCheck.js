...
export function useVersionCheck() {
  ...
  const getCurrentVersion = async (useCache = true) => {
    if (
      useCache &&
      versionCache.value &&
      Date.now() - lastCheckTime < CHECK_COOLDOWN
    ) {
    ...
    try {
      ...
      if (dockerResponse.data.status === "success") {
        ...
        currentImageTag.value =
          version.includes("nightly") || version.includes("dev")
            ? "nightly"
            : "latest";
      }
  ...
  const showUpdateNotification = () => {
    ...
    toast.info(message, {
      ...
      onClick: () => {
        if (isNightly) {
          window.open(
            "https://hub.docker.com/r/ciuse99/suggestarr/tags",
            "_blank",
          );
        } else {
          window.open(
            "https://github.com/giuseppe99barchetta/SuggestArr/releases/latest",
            "_blank",
          );
        }
      },
