<template>
  <section class="request-workflow">
    ...
    <div class="suggestion-toolbar">
      <input v-model.trim="search" type="search" placeholder="Search title or TMDb ID" aria-label="Search requests" @input="reloadSoon" />
...
export default {
  ...
  methods: {
    async load(page = this.page) { this.loading = true; try { const data = (await axios.get('/api/automation/requests/workflow', { params: { status: this.status, search: this.search, page, per_page: 24 } })).data; Object.assign(this, { items: data.items, total: data.total, page: data.page, pages: data.pages, selected: [] }); } finally { this.loading = false; } },
