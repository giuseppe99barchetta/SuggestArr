  ...
  <section class="request-workflow">
    ...
    <div v-else class="requests-grid">
      <article v-for="item in items" :key="item.id" class="request-card card card--column card--interactive card--padding-none" :class="{ 'card--selected': selected.includes(item.id) }" tabindex="0" @click="openItem(item)" @keydown.enter="openItem(item)">
        ...
        <div class="request-card-body"><h3 class="request-card-title">{{ item.title || `TMDb ${item.tmdb_id}` }}</h3><div class="badge-container"><span class="badge badge-media">{{ item.media_type.toUpperCase() }}</span><span class="badge badge-rating"><i class="fas fa-star"></i> {{ formatRating(item.rating) }}</span><span class="badge badge-requested">{{ statusLabel(item.status) }}</span></div><div class="source-link"><span>From: <strong>{{ item.name }}</strong></span></div><div v-if="item.user_name || item.media_user_id" class="source-link"><i class="fas fa-user"></i><span>For: <strong>{{ item.user_name || item.media_user_id }}</strong></span></div><div class="feedback-control" @click.stop><label :for="`feedback-${item.id}`">My feedback</label><select :id="`feedback-${item.id}`" :value="item.feedback?.feedback || ''" :disabled="actionLoading" @change="setFeedback(item, $event.target.value)"><option value="">No feedback</option><option value="interested">Interested</option><option value="not_interested">Not interested</option><option value="already_seen">Already seen</option><option value="too_similar">Too similar</option><option value="save_for_later">Save for later</option></select><select v-if="item.feedback?.feedback" :value="item.feedback.reason_type || ''" :disabled="actionLoading" aria-label="Feedback reason" @change="setFeedback(item, item.feedback.feedback, $event.target.value)"><option value="">No reason</option><option value="genre">Genre</option><option value="provider">Provider</option><option value="content">Content</option><option value="title">Title</option><option value="other">Other</option></select></div><small v-if="item.status === 'failed'">{{ item.last_error || `Failed after ${item.retry_count} attempts` }}</small></div>
      </article>
    </div>
...
export default {
  ...
  methods: {
    ...
    async setFeedback(item, feedback, reasonType) { this.actionLoading = true; try { if (feedback) { const payload = { feedback }; if (reasonType !== undefined) payload.reason_type = reasonType || null; const response = await axios.put(`/api/automation/requests/workflow/${item.id}/feedback`, payload); item.feedback = response.data.feedback; } else { await axios.delete(`/api/automation/requests/workflow/${item.id}/feedback`); item.feedback = null; } this.$toast.open({ message: feedback ? 'Feedback saved' : 'Feedback cleared', type: 'success' }); } catch (error) { this.$toast.open({ message: error.response?.data?.message || 'Unable to save feedback', type: 'error' }); await this.load(1); } finally { this.actionLoading = false; } },
...
.feedback-control{display:flex;flex-wrap:wrap;align-items:center;gap:var(--spacing-sm);margin-top:var(--spacing-sm)}.feedback-control label{color:var(--color-text-secondary);font-size:var(--font-size-sm)}.feedback-control select{min-width:0}
